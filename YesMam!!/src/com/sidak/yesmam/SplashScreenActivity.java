	...
	protected void onResume() {
		...
		if (!isSet) {
			...
			runDelayedNextActivity(SemesterActivity.class);
			  ...
			  wDataSource.open(); 
			  String lastDateString=wdayPrefs.getString(getString(R.string.last_date), "01/01/2000");
			  String currentDateString=UIHelper.getCurrentDate(); 
			  Date lastDate=UIHelper.getDateObjectFromText(lastDateString); 
			  Date currentDate=UIHelper.getDateObjectFromText(currentDateString);
			  if(!(lastDateString.equals("01/01/2000"))){
				  if(lastDate.before(currentDate)){ 
				  // get the no of working days between the last updated date and date before current date
					  SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); 
					  String lastDateFormatted = formatter.format(lastDate); 
					  String currentDateFormatted = formatter.format(currentDate);
					  wdays=wDataSource.findBeforeToday(lastDateFormatted,currentDateFormatted);
					  
					  // in each working day , find out the courses which have classes on that day 
					  // then for that course check if attendance was marked
					  // if no attendance marked , then add the course-code,date of class to the list of leftCourseCodes 
					  // if marked then in a local hashmap corresponiding to the course code - 
					  ...
					  // initialise the codes and dates list which will be used to store data for classes
					  ...
					  for(WorkingDay wd : wdays){
						  ...
						  for(int i=0; i<len; i++){
							  // if corresponding to that course ,there was a class but attendance not marked
							  String code = wd.codes.get(i);
							  if(wd.attendance.get(i)==2){
								  
								  UnmarkedClass uc = new UnmarkedClass(wd.getDateString(),
										  courseDataSource.getCourseWithCode(code),2);
					  ...
					  // store the value to be added in the fields of num_attendance and num_bunk 
					  // for each of the course which have had atleat one class, 
					  // update the value in the database - load initially and then update for each of the course above 
					  
					  
					  for (Map.Entry<String, Integer> entry : attend.entrySet()){
					      //System.out.println(entry.getKey() + "/" + entry.getValue());
						  if(entry.getValue()>0){
							  //load that course from course database 
							  // then update it's value of num_attend field
							  Course course = courseDataSource.getCourseWithCode(entry.getKey());
							  int numAttend= course.getAttendedClasses()+entry.getValue();
							  courseDataSource.markPresent(numAttend, entry.getKey());
						  }
					  }
					  ...
					  for (Map.Entry<String, Integer> entry : bunk.entrySet()){
						  ...
						  if(entry.getValue()>0){
							  ...
							  Course course = courseDataSource.getCourseWithCode(entry.getKey());
							  int numBunk= course.getBunkedClasses()+entry.getValue();
							  courseDataSource.markAbsent(numBunk, entry.getKey());
						  }
					  }
			  ...
			  // then after the above operation is done, check if list of leftCourses is not empty 
			  if(!unmarkedClasses.isEmpty()){
				  // make a model class for leftover classes and then 
				  // if not empty, then open an activity containing the left Classes
